...
function drawLayer(canvas, layer){
	for(k=0; k<layer.tiles.length; k++){
		if(layer.tiles[k].tsX>=0){
			canvas.drawImage(layer.tileset.image, layer.tiles[k].tsX, layer.tiles[k].tsY, layer.tiles[k].tsWidth, layer.tiles[k].tsHeight, layer.tiles[k].mapX, layer.tiles[k].mapY, layer.tiles[k].tsWidth, layer.tiles[k].tsHeight);
		}
	}
}
